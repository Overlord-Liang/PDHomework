...
void Widget::setupSidebar(Sidebar *parent)
{
    ...
    // 设置选中提示线颜色
    btn_page_1->setPromptLineColor(QColor(87, 199, 237));
    btn_page_2->setPromptLineColor(QColor(35, 176, 172));
    btn_page_3->setPromptLineColor(QColor(235, 103, 51));
    btn_set->setPromptLineColor(QColor(113, 124, 134));

    // 设置按钮图标
    btn_page_1->setIcon(QPixmap(":/button_icon/Account.svg"));
    btn_page_2->setIcon(QPixmap(":/button_icon/Schedual.svg"));
    btn_page_3->setIcon(QPixmap(":/button_icon/Chat.svg"));
    btn_set->setIcon(QPixmap(":/button_icon/gear.svg"));
    btn_expand->setIcon(QPixmap(":/button_icon/three_lines.svg"));

    // 设置右侧文本
    btn_page_1->setText("这是第一个页面哦");
    btn_page_2->setText("这是第二个页面哦");
    btn_page_3->setText("这是第三个页面哦");
    btn_set->setText("设置");
...
void Widget::setupStackedWidget(QStackedWidget *parent)
{
    ...
    page_1->findChildren<QLabel *>().at(0)->setText("这是第一个页面哦~");
    page_2->findChildren<QLabel *>().at(0)->setText("这是第二个页面哦~");
    page_3->findChildren<QLabel *>().at(0)->setText("这是第三个页面哦~");
    page_set->findChildren<QLabel *>().at(0)->setText("设置啦~");
